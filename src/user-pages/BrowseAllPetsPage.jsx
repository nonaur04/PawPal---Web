        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6">
          <div className="mx-auto" style={{ maxWidth: 1100 }}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-2">
              ...
              <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl w-full sm:w-[260px]" style={{ backgroundColor: "white", border: "1px solid #EEE8E0" }}>
                <span className="text-gray-400 text-sm">🔍</span>
                <input
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  placeholder="Search name, breed, area"
                  className="flex-1 bg-transparent text-sm outline-none placeholder-gray-300 min-w-0"
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
                {filtered.map((p) => (
                  <PetCard key={p.id} pet={toCardPet(p)} fluid />
                ))}
              </div>
            )}
          </div>
