    ...
    <div className="flex items-start gap-4 p-4 rounded-2xl" style={{ backgroundColor: "white", border: "1px solid #EEE8E0" }}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          ...
          <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: "#FFF3E0", color: "#F5A623" }}>You</span>
        </div>
...
export default function MyStrayReportsPage() {
  ...
  const active = reports.filter((r) => r.status === "in_progress" || r.status === "open" || r.status === "pending");
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6 max-w-2xl">
            {[
              { value: reports.length, label: "Reported" },
              { value: active.length, label: "Active" },
              { value: resolved.length, label: "Resolved" },
            ].map((s) => (
          ...
          {loading ? (
            ...
            <div className="rounded-2xl p-8 text-center max-w-md" style={{ backgroundColor: "white", border: "1px solid #EEE8E0" }}>
              ...
              <button onClick={() => navigate("/reports/new-stray")}
            ...
            <div className="space-y-3 max-w-3xl">
              {reports.map((r) => <StrayRow key={r.id} report={r} />)}
            </div>
          )}
        </main>
      </div>
