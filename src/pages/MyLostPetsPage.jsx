      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6 max-w-2xl">
            {[
              { value: reports.length, label: "Posted" },
              { value: searching.length, label: "Searching" },
              { value: reunited.length, label: "Reunited" },
            ].map((s) => (
          ...
          {loading ? (
            <div className="grid grid-cols-3 gap-4">
              {[1, 2, 3].map((i) => <div key={i} className="rounded-2xl animate-pulse" style={{ height: 340, backgroundColor: "white" }} />)}
            </div>
          ) : reports.length === 0 ? (
            <div className="rounded-2xl p-8 text-center max-w-sm" style={{ backgroundColor: "white", border: "1px solid #EEE8E0" }}>
              ...
              <button onClick={() => navigate("/reports/new-lost")}
                className="px-5 py-2.5 rounded-xl text-sm font-bold text-white" style={{ backgroundColor: "#F5A623" }}>
                + New post
              </button>
            </div>
          ) : (
            <div className="grid grid-cols-3 gap-4">
              {reports.map((r) => <LostCard key={r.id} report={r} />)}
            </div>
          )}
        </main>
      </div>
